...
SerialData parseCrc(const QByteArray& inputByteArray) {
    ...
    // 解析Address（前两个字节）
    parsedData.addr = QString("%1").arg(static_cast<quint8>(inputByteArray[0]), 2, 16, QLatin1Char('0')).toUpper();

    // 解析Code（第三个字节）
    parsedData.code = QString("%1").arg(static_cast<quint8>(inputByteArray[1]), 2, 16, QLatin1Char('0')).toUpper();

    qint16 status = (static_cast<quint8>(inputByteArray[2]) << 8) + static_cast<quint8>(inputByteArray[3]);
    quint16 data_len = status & 0x0FFF;

    // 解析Quantity（第四和第五个字节）
    parsedData.quantity = QString("%1").arg(static_cast<quint16>(status), 4, 16, QLatin1Char('0')).toUpper();
...
void SingletonManager::receive()
{
    ...
    while (startIndex != -1) {
        ...
        if (endIndex != -1) {
            // 提取一帧内容
            QByteArray frameData = buffer.mid(startIndex + 1, endIndex - startIndex - 1);
            ...
            auto processor = [frameData, this]() {
                ...
                log.append("Receive: Hex:       "+utils::formatQByte(encode)+" 解密前\n");
                ...
                if(s.circle) {
                    log.append(QString("Receive: Data:      Address:%1   Code:%2   Quantity:%3   TimeStamp:%4   Data:%5\n").arg(s.addr, s.code, s.quantity, s.time, s.data));
                } else {
...
void SingletonManager::showGlobalToast(QString msg) {
    emit showToast(msg);
}

QString SingletonManager::sendData(QString addr, QString code, QString data, bool circle)
{
    addr.replace(" ", "");
    code.replace(" ", "");
    data.replace(" ", "");
    ...
    uint32_t timeS = static_cast<uint32_t>(currentDateTime.toMSecsSinceEpoch() / 1000);
    ...
    s_data.append(code);
    //状态位和数据长度
    s_data.append(QString("%1").arg(len & 0x0FFF, 4, 16, QLatin1Char('0')).toUpper());
    //时间戳
    s_data.append(time);
    // 数据
    s_data.append(data);
